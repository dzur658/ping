...
def insert_data_from_json(conn, json_data):
    ...
    try:
        ...
        for ip_address, host_data in json_data.items():
            ...
            # look for custom nmap scripts
            hostscripts = host_data.get('hostscripts', {})
            if hostscripts:
                for entry in hostscripts.values():
                    ...
                    oui = entry.get('oui', '')
                    port_risk = entry.get('risk', '')
                    oui_detect = entry.get('device', '')
                    mitigation = entry.get('mitigation', '')

                    cursor.execute("""
                        INSERT INTO detection (hostId, openPorts, manufacturer, oui, portRisk, ouiDetect, mitigation)
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                    """, (host_id, open_ports, manufacturer, oui, port_risk, oui_detect, mitigation))
